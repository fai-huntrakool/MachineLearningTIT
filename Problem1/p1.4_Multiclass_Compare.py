...
n=200
x1=3*(np.random.rand(n,4)-0.5)
W=np.array([[2,-1,0.5],[-3,2,1],[1,2,3]])

t=np.dot(np.hstack((x1[:,0:2],np.ones((n,1)))),W.transpose())+0.5*np.random.randn(n,3)
maxlogit=np.max(t,axis=1)
y=np.argmax(t,axis=1)


classes=[0,1,2]
x=np.hstack([x1,np.ones((n,1))])
...
while(t<epoch):
    ...
    grad=grad/n+2*lam*w
    J_w+=jw/n+lam*np.sum(np.dot(w.T,w))
    loss_history2.append(J_w)
    for j in classes:
        w[:,j]=w[:,j]-np.dot(np.linalg.inv(hess[j]),grad[:,j])*alpha
    t+=1
...
minLoss1=min(loss_history1)
minLoss2=min(loss_history2)
overall_min=max(minLoss1,minLoss2)

for i in range(len(loss_history1)):
    diff1.append(loss_history1[i]-overall_min)

for i in range(len(loss_history2)):
    diff2.append(loss_history2[i]-overall_min)
