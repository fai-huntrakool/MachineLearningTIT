...
#SMO
def f(p,alpha,y):
    return np.sum([alpha[i]*y[0,i]*np.dot(p[i].T,p) for i in range(0,len(p))])
...
count=0
while count<epoch:
    alpha_hist3.append(alpha)
    for i in range(0,x.shape[1]):
        j=i
        while j==i:
            j=np.random.randint(0,x.shape[1]-1)
        
        E1=f(x[:,i],alpha,y)-y[0,i]
        E2=f(x[:,j],alpha,y)-y[0,j]
